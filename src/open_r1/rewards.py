...
def accuracy_reward(completions, solution, **kwargs):
    ...
    rewards = []
    sol_num = len(set(solution))  # 需要找到sol的个数
    num_for_each_contents = kwargs.get("num_for_each_contents")
    batch_rewards = []
    batch_length = []
    for index, (content, sol) in enumerate(zip(contents, solution)):
        gold_parsed = parse(
            sol,
        ...
        if len(gold_parsed) != 0:
            ...
            try:
                ...
                batch_length.append(content_length)
        ...
        if (index + 1) % num_for_each_contents == 0:
            batch_rewards = np.array(batch_rewards)
            batch_length = np.array(batch_length)
            if sum(batch_rewards) != 0:
                # 创建掩膜标识哪些位置需要计算（reward == 1）
                mask = (batch_rewards == 1.0)
                # 仅对需要计算的位置进行处理
                denominator = 1 + np.exp(-alpha * (np.mean(batch_length[mask]) - batch_length[mask]))
                calculated_rewards = np.clip(batch_rewards[mask] / denominator, 0.5, 1.0)
                batch_rewards[mask] = calculated_rewards
...
def accuracy_absolute_reward(completions, solution, **kwargs):
